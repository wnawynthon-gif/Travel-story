...
const PUBLISH_FEED_SCHEMA={type:"object",additionalProperties:false,properties:{facebook:{type:"object",additionalProperties:false,properties:{post:{type:"string"},hashtags:{type:"array",minItems:3,maxItems:10,items:{type:"string"}},cta:{type:"string"}},required:["post","hashtags","cta"]},instagram:{type:"object",additionalProperties:false,properties:{caption:{type:"string"},hashtags:{type:"array",minItems:5,maxItems:15,items:{type:"string"}},cover_text:{type:"string"},carousel_slides:{type:"array",minItems:3,maxItems:7,items:{type:"string"}}},required:["caption","hashtags","cover_text","carousel_slides"]}},required:["facebook","instagram"]};
const PUBLISH_SHORTS_SCHEMA={type:"object",additionalProperties:false,properties:{instagram_reel:{type:"object",additionalProperties:false,properties:{caption:{type:"string"},hook:{type:"string"},cover_text:{type:"string"},hashtags:{type:"array",minItems:5,maxItems:12,items:{type:"string"}}},required:["caption","hook","cover_text","hashtags"]},tiktok:{type:"object",additionalProperties:false,properties:{caption:{type:"string"},hook:{type:"string"},hashtags:{type:"array",minItems:4,maxItems:10,items:{type:"string"}}},required:["caption","hook","hashtags"]},youtube_shorts:{type:"object",additionalProperties:false,properties:{title:{type:"string"},description:{type:"string"},hook:{type:"string"},hashtags:{type:"array",minItems:3,maxItems:8,items:{type:"string"}}},required:["title","description","hook","hashtags"]}},required:["instagram_reel","tiktok","youtube_shorts"]};
...
Type must be FACT, HISTORY, TRUE_STORY, LEGEND, FOLKLORE, MIXED, or DISPUTED. Never present folklore as fact. Keep proper nouns in common local/English spelling. Be concise. No markdown.`;
  ...
  if(action==="PUBLISH_FEED")return res.status(200).json(await ask(`${contentBase} Existing social content: ${JSON.stringify(context.content||{})}. Create a ready-to-copy publishing pack in Thai for Facebook feed and Instagram feed/carousel. Facebook should read naturally as a story post, not hashtag spam. Instagram should have a strong first line, concise caption, cover text, and 3-7 carousel slide texts. Keep legend/folklore/disputed claims clearly labelled. No markdown.`,PUBLISH_FEED_SCHEMA,"travel_publish_feed_v197",{fast:true,timeout:42000}));
  if(action==="PUBLISH_SHORTS")return res.status(200).json(await ask(`${contentBase} Existing AI video pack: ${JSON.stringify(context.video||{})}. Existing social content: ${JSON.stringify(context.content||{})}. Create ready-to-copy Thai publishing metadata for Instagram Reel, TikTok, and YouTube Shorts. Optimize the hook and wording for each platform without clickbait or changing factual caveats. Keep legend/folklore/disputed claims clearly labelled. No markdown.`,PUBLISH_SHORTS_SCHEMA,"travel_publish_shorts_v197",{fast:true,timeout:42000}));
