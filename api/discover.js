...
const MAP_SCHEMA={type:"object",additionalProperties:false,properties:{map_summary:{type:"string"},stops:{type:"array",minItems:1,maxItems:5,items:{type:"object",additionalProperties:false,properties:{name:{type:"string"},location:{type:"string"},reason:{type:"string"}},required:["name","location","reason"]}},route_note:{type:"string"}},required:["map_summary","stops","route_note"]};
...
Never present folklore as established fact. No markdown.`;
  ...
  if(action==="ILLUSTRATE")return res.status(200).json(await ask(`${base} Create an editorial illustration art direction and detailed image-generation prompt. Respect architecture, era, clothing and cultural details; list must-include and avoid items. No markdown.`,ILLUSTRATE_SCHEMA,"travel_illustrate_final"));
  if(action==="MAP")return res.status(200).json(await ask(`${base} Create a compact map/story route using only real location wording supported by the selected story/context. Never invent coordinates or addresses. No markdown.`,MAP_SCHEMA,"travel_map_final"));
