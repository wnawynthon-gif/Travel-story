...
if(tab==='stories')html=`<div class="grid">${arr(g.stories).map(s=>`<div class="card story wide"><small>${esc(s.type)}</small><h3>${esc(s.title)}</h3>${s.hook?`<p class="story-hook"><b>${esc(s.hook)}</b></p>`:''}<div class="story-engine"><span class="tag">📍 Place</span> → <span class="tag">📰 Current Event</span> → <span class="tag">💎 Famous Object</span> → <span class="tag">📜 Backstory</span> → <span class="tag">👤 Human Drama</span> → <span class="tag">🔗 Connection</span> → <span class="tag">🚶 Visit Today</span></div><p>${esc(s.story)}</p>${s.place?`<p><b>📍 Place:</b> ${esc(s.place)}</p>`:''}${s.current_event?`<p><b>📰 Current Event:</b> ${esc(s.current_event)}</p>`:''}${s.famous_object?`<p><b>💎 Famous Object:</b> ${esc(s.famous_object)}</p>`:''}${s.historical_backstory?`<p><b>📜 Historical Backstory:</b> ${esc(s.historical_backstory)}</p>`:''}${s.human_drama?`<p><b>👤 Human Drama:</b> ${esc(s.human_drama)}</p>`:''}${s.unexpected_connection?`<p><b>🔗 Unexpected Connection:</b> ${esc(s.unexpected_connection)}</p>`:''}${s.present_day_visit?`<p><b>🚶 Present-day Visit:</b> ${esc(s.present_day_visit)}</p>`:''}<span class="tag">📍 ${esc(s.area)}</span><span class="tag">✓ ${esc(s.verification)}</span>${arr(s.timeline).length?`<h4>🕰 Timeline</h4><ul>${arr(s.timeline).map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:''}${arr(s.key_people_objects).length?`<p><b>👤 People / Objects:</b> ${arr(s.key_people_objects).map(esc).join(' • ')}</p>`:''}${s.place_connection?`<p><b>🔗 Connection to this place:</b> ${esc(s.place_connection)}</p>`:''}${s.verification_note?`<p><b>🔎 Verification note:</b> ${esc(s.verification_note)}</p>`:''}<p><b>Why it matters:</b> ${esc(s.why_it_matters)}</p>${arr(s.sources).length?`<h4>🌐 Sources</h4><ul>${arr(s.sources).map(x=>`<li><a href="${esc(x.url)}" target="_blank" rel="noopener noreferrer">${esc(x.title||x.publisher||'Source')}</a>${x.publisher?` — ${esc(x.publisher)}`:''}${x.date?` (${esc(x.date)})`:''}</li>`).join('')}</ul>`:''}${arr(s.fact_check_search_terms).length?`<p><b>Search to verify / expand:</b> ${arr(s.fact_check_search_terms).map(x=>`<span class="tag">${esc(x)}</span>`).join(' ')}</p>`:''}${illBtn({name:s.title,story:s.story,type:s.type,area:s.area,verification:s.verification})}</div>`).join('')}</div>`;
